...

{
  ...
  gPad->SetLogy(1);
  copy->SetTitle("Simulated Spill (Patholgical);Time in Spill            (ms); Protons per Pulse");

  canvas->cd(2);
  copy =hSimFRMS->DrawCopy("HIST");
  line->DrawLine( t0, four_percent, tend, four_percent);
  gPad->SetLogy(1);
  copy->SetTitle("Simulated Spill (Pathological) Rebinned by 10;Time in Spill            (ms); RMS/Mean");
